...
const JobCard: React.FC<JobCardProps> = ({ job }) => {
  ...
  const toggleFavorite = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    setIsSaved(!isSaved);
    // В реальном приложении здесь был бы код для сохранения в localStorage или на сервере
    const savedJobs = JSON.parse(localStorage.getItem('savedJobs') || '[]');
    if (!isSaved) {
      localStorage.setItem('savedJobs', JSON.stringify([...savedJobs, job]));
    } else {
      localStorage.setItem('savedJobs', JSON.stringify(savedJobs.filter((j: Job) => j.id !== job.id)));
    }
  };
